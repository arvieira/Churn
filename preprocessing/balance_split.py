...
def base_balance(x_train, y_train, alg='random'):
    return balance(x_train, y_train, alg)
...
def balance(df_X, df_y, alg):
    if alg == 'random':
        ...
        print(f"-> Baleanceando a base. Método escolhido: TomekLink + SMOTE...")
        smote_sampler = SMOTETomek(tomek=TomekLinks(sampling_strategy='majority'))
        return smote_sampler.fit_resample(df_X, df_y)
    else:
...
def base_split(df):
    ...
    print("-> Separando a base em treino e teste com proporção de 80/20...")
    df_X_train, df_X_test, df_y_train, df_y_test = train_test_split(X, y, test_size=0.20, stratify=y, random_state=SEED)
